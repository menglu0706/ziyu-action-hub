...
export async function getAdminInitialData(scope:AdminDataScope='tasks',client?:Awaited<ReturnType<typeof createClient>>):Promise<AdminInitialData>{
  ...
  return {
    tasks:(tasksResult.data??[]).map((r,i)=>({id:r.id,title:r.title,category:r.category,platform:r.platform??'',urgency:urgencyLabel(r.urgency_score),required:requiredLabel(r.required_score),minutes:r.estimated_minutes,deadline:r.deadline?new Date(r.deadline).toLocaleString('zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):'—',status:statusLabel(r.status),url:r.external_url,quick:r.quick_instruction,description:r.description??'',recommendedCopy:r.recommended_copy??'',rawDeadline:r.deadline,audience:r.audience,pinned:r.is_pinned,daily:r.show_in_daily,multi:r.is_multi_account,history:r.count_completion,leader:r.leaderboard_enabled,push:r.push_reserved,urgentSortPosition:r.urgent_sort_position??null,_sort:i} as AdminTask & {_sort:number})),

...
export type AdminDashboardStats={todayVisits:number;todayTaskCompletions:number;registeredUsers:number;currentUrgentTasks:number};
...
export async function getAdminInitialData(scope:AdminDataScope='tasks',client?:Awaited<ReturnType<typeof createClient>>):Promise<AdminInitialData>{
  ...
  return {
    ...
    stats:statsRow?{todayVisits:Number(statsRow.today_visits),todayTaskCompletions:Number(statsRow.today_task_completions),registeredUsers:Number(statsRow.registered_users),currentUrgentTasks:Number(statsRow.current_urgent_tasks)}:null,

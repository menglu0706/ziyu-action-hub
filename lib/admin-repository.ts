...
export type AdminDataScope='tasks'|'links'|'templates'|'media'|'guides'|'visual'|'none';
...
export async function getAdminInitialData(scope:AdminDataScope='tasks',client?:Awaited<ReturnType<typeof createClient>>):Promise<AdminInitialData>{
  ...
  const [tasksResult,linksResult,templatesResult,mediaResult,guidesResult,visualResult]=await Promise.all([
    scope==='tasks'?db.from('tasks').select('*').order('is_pinned',{ascending:false}).order('urgent_sort_position',{ascending:true,nullsFirst:false}).order('created_at',{ascending:false}):empty(),

...
const mapAdminTask=(r:any,i:number)=>({id:r.id,title:r.title,category:r.category,platform:r.platform??'',urgency:urgencyLabel(r.urgency_score),required:requiredLabel(r.required_score),minutes:r.estimated_minutes,deadline:r.deadline?new Date(r.deadline).toLocaleString('zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):'—',status:statusLabel(r.status),url:r.external_url,quick:r.quick_instruction,description:r.description??'',recommendedCopy:r.recommended_copy??'',rawDeadline:r.deadline,audience:r.audience,pinned:r.is_pinned,urgentVisible:r.show_in_urgent,heat:r.show_in_heat??false,daily:r.show_in_daily,dailyGroup:r.daily_group,source:r.source??'manual',push:r.push_reserved,urgentSortPosition:r.urgent_sort_position??null,dailySortPosition:r.daily_sort_position??null,publishAt:r.publish_at??null,pinStartsAt:r.pin_starts_at??null,pinEndsAt:r.pin_ends_at??null,_sort:i} as AdminTask & {_sort:number});

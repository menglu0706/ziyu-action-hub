...
export async function getAdminInitialData(scope:AdminDataScope='tasks',client?:Awaited<ReturnType<typeof createClient>>):Promise<AdminInitialData>{
  ...
  if(tasksResult.error||linksResult.error||templatesResult.error||mediaResult.error||guidesResult.error||visualResult.error)return {tasks:[],links:[],templates:[],media:[],guides:[],visual:[],stats:null,error:'数据加载失败，请检查数据库迁移后重试。'};
  const statsRow=Array.isArray(statsResult.data)?statsResult.data[0]:statsResult.data;
  const completionTrend=Array.isArray(statsRow?.completion_trend)?statsRow.completion_trend.map((item:{date:unknown;count:unknown})=>({date:String(item.date),count:Number(item.count)})):[];
  return {
    tasks:(tasksResult.data??[]).map((r,i)=>({id:r.id,title:r.title,category:r.category,platform:r.platform??'',urgency:urgencyLabel(r.urgency_score),required:requiredLabel(r.required_score),minutes:r.estimated_minutes,deadline:r.deadline?new Date(r.deadline).toLocaleString('zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):'—',status:statusLabel(r.status),url:r.external_url,quick:r.quick_instruction,description:r.description??'',recommendedCopy:r.recommended_copy??'',rawDeadline:r.deadline,audience:r.audience,pinned:r.is_pinned,urgentVisible:r.show_in_urgent,daily:r.show_in_daily,multi:r.is_multi_account,history:r.count_completion,leader:r.leaderboard_enabled,push:r.push_reserved,urgentSortPosition:r.urgent_sort_position??null,dailySortPosition:r.daily_sort_position??null,_sort:i} as AdminTask & {_sort:number})),
    ...
    stats:statsRow?{todayVisits:Number(statsRow.today_visits),todayTaskCompletions:Number(statsRow.today_task_completions),registeredUsers:Number(statsRow.registered_users),currentUrgentTasks:Number(statsRow.current_urgent_tasks),completionTrend}:null,

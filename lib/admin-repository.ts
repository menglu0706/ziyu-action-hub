...
export async function getAdminInitialData():Promise<AdminInitialData>{
  ...
  const [tasksResult,linksResult,templatesResult,mediaResult,guidesResult,visualResult]=await Promise.all([
    db.from('tasks').select('*').order('is_pinned',{ascending:false}).order('urgent_sort_position',{ascending:true,nullsFirst:false}).order('created_at',{ascending:false}),db.from('quick_links').select('*').order('sort_order'),db.from('text_templates').select('*').order('sort_order'),db.from('media_items').select('*').order('published_at',{ascending:false}),db.from('guides').select('*').order('sort_order'),db.from('visual_settings').select('*').order('module_key')
  ]);
  ...
  return {
    tasks:(tasksResult.data??[]).map((r,i)=>({id:r.id,title:r.title,category:r.category,urgency:urgencyLabel(r.urgency_score),required:requiredLabel(r.required_score),minutes:r.estimated_minutes,deadline:r.deadline?new Date(r.deadline).toLocaleString('zh-CN',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):'—',status:statusLabel(r.status),url:r.external_url,quick:r.quick_instruction,description:r.description??'',recommendedCopy:r.recommended_copy??'',rawDeadline:r.deadline,audience:r.audience,pinned:r.is_pinned,daily:r.show_in_daily,multi:r.is_multi_account,history:r.count_completion,leader:r.leaderboard_enabled,push:r.push_reserved,urgentSortPosition:r.urgent_sort_position??null,_sort:i} as AdminTask & {_sort:number})),

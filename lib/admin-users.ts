...
export async function getAdminAccounts():Promise<AdminAccountsResult>{
  try{
    ...
    if(rowsResult.error)return {accounts:[],error:`后台账号加载失败（${rowsResult.error.code}）：${rowsResult.error.message}`};
    ...
    if(authError)return {accounts:[],error:`后台账号加载失败：${authError.message}`};

...
export type Task={id:string;title:string;platform:string;category:Category;urgency:number;required:number;minutes:number;deadline:string|null;createdAt?:string;quick:string;description:string;recommendedCopy:string;url:string;pinned:boolean;daily:boolean;multi:boolean;completionMode:'one_time'|'daily';urgentSortPosition:number|null;progress:number;goal:number|null;completed?:boolean;steps:string[]};
export type LeaderboardRow={userId:string;nickname:string;avatarUrl:string|null;actionCount:number;rank:number;periodStart:string;periodEnd:string};

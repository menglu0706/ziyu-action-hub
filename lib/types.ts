...
export type Task={id:string;title:string;platform:string;category:Category;urgency:number;required:number;minutes:number;deadline:string|null;createdAt?:string;quick:string;description:string;url:string;pinned:boolean;daily:boolean;multi:boolean;completionMode:'one_time'|'daily';progress:number;goal:number|null;completed?:boolean;steps:string[]};

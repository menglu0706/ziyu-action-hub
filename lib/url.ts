...
export function weiboWboxNativeUrl(value:string){const original=externalWebHref(value);if(!original||!isWeiboWboxUrl(original))return null;const query=new URL(original).search.slice(1);return query?`sinaweibo://wbox?${query}`:null}
export function weiboWboxLandingUrl(value:string){const native=weiboWboxNativeUrl(value);if(!native)return null;const landing=new URL('https://m.s.weibo.com/applink/nativeqrcode');landing.searchParams.set('url',native);return landing.toString()}

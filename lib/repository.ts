...
type TaskRow={id:string;title:string;description:string|null;recommended_copy:string|null;category:string;platform:string|null;external_url:string;quick_instruction:string;urgency_score:number;required_score:number;estimated_minutes:number;deadline:string|null;created_at:string;is_pinned:boolean;show_in_daily:boolean;is_multi_account:boolean;completion_mode:'one_time'|'daily';sort_order:number;urgent_sort_position:number|null;task_steps?:{step_number:number;content:string}[]};
...
const getCachedUrgentTasks=unstable_cache(async()=>{const db=createPublicClient();const now=new Date().toISOString();const rows=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('status','published').or(`deadline.is.null,deadline.gt.${now}`).order('is_pinned',{ascending:false}).order('urgent_sort_position',{ascending:true,nullsFirst:false}).order('created_at',{ascending:false}));return (rows??[]).map(row=>mapTask(row as TaskRow))},['public-urgent-tasks'],taskCacheOptions);

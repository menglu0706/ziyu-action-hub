...
const getCachedTasks=unstable_cache(withFallback(async()=>{const db=createPublicClient();const rows=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('status','published').eq('show_in_daily',true).order('is_pinned',{ascending:false}).order('daily_sort_position',{ascending:true,nullsFirst:false}).order('deadline',{ascending:true,nullsFirst:false}).order('urgency_score',{ascending:false}).order('required_score',{ascending:false}).order('sort_order'));return (rows??[]).map(row=>mapTask(row as TaskRow))},[] as Task[]),['public-tasks'],{...taskCacheOptions,tags:[DAILY_TASKS_CACHE_TAG]});
const getCachedUrgentTasks=unstable_cache(withFallback(async()=>{const db=createPublicClient();const now=new Date().toISOString();const rows=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('status','published').eq('show_in_urgent',true).or(`deadline.is.null,deadline.gt.${now}`).order('is_pinned',{ascending:false}).order('urgent_sort_position',{ascending:true,nullsFirst:false}).order('created_at',{ascending:false}));return (rows??[]).map(row=>mapTask(row as TaskRow))},[] as Task[]),['public-urgent-tasks'],taskCacheOptions);
...
const getCachedMedia=unstable_cache(withFallback(async()=>{const db=createPublicClient();const cutoff=new Date(Date.now()-MEDIA_EXPIRY_MS).toISOString();const rows=await run(db.from('media_items').select('*').eq('is_enabled',true).gte('published_at',cutoff).order('published_at',{ascending:false}));return ((rows??[]) as MediaRow[]).map(r=>({id:r.id,title:r.title,category:r.category,publishedAt:r.published_at,coverUrl:r.cover_url??'',url:r.external_url,isNew:r.is_new,enabled:r.is_enabled}))},[] as MediaItem[]),['public-media'],publicCacheOptions);

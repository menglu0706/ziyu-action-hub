...
const getCachedMedia=unstable_cache(withFallback(async()=>{const db=createPublicClient();const rows=await run(db.from('media_items').select('*').eq('is_enabled',true).order('published_at',{ascending:false}));return ((rows??[]) as MediaRow[]).map(r=>({id:r.id,title:r.title,category:r.category,publishedAt:r.published_at,coverUrl:r.cover_url??'',url:r.external_url,isNew:r.is_new,enabled:r.is_enabled}))},[] as MediaItem[]),['public-media'],publicCacheOptions);

...
type TaskRow={id:string;title:string;description:string|null;recommended_copy:string|null;category:string;platform:string|null;external_url:string;quick_instruction:string;urgency_score:number;required_score:number;estimated_minutes:number;deadline:string|null;created_at:string;is_pinned:boolean;show_in_urgent:boolean;show_in_heat:boolean;show_in_daily:boolean;daily_group:DailyGroup;completion_mode:'one_time'|'daily';sort_order:number;urgent_sort_position:number|null;task_steps?:{step_number:number;content:string}[]};
const mapTask=(row:TaskRow):Task=>({id:row.id,title:row.title,description:row.description??'',recommendedCopy:row.recommended_copy??'',category:row.category as Category,platform:row.platform??'',url:row.external_url,quick:row.quick_instruction,urgency:row.urgency_score,required:row.required_score,minutes:row.estimated_minutes,deadline:row.deadline,createdAt:row.created_at,pinned:row.is_pinned,urgent:row.show_in_urgent,heat:row.show_in_heat,daily:row.show_in_daily,dailyGroup:row.daily_group,urgentSortPosition:row.urgent_sort_position??null,steps:[...(row.task_steps??[])].sort((a,b)=>a.step_number-b.step_number).map(x=>x.content)});
...
export const TASK_DETAIL_CACHE_TAG='public-task-detail';

const getCachedTasks=unstable_cache(withFallback(async()=>{const db=createPublicClient();const now=new Date().toISOString();const rows=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('status','published').eq('show_in_daily',true).or(`deadline.is.null,deadline.gt.${now}`).order('is_pinned',{ascending:false}).order('daily_sort_position',{ascending:true,nullsFirst:false}).order('deadline',{ascending:true,nullsFirst:false}).order('urgency_score',{ascending:false}).order('required_score',{ascending:false}).order('sort_order'));return dedupeBy((rows??[]).map(row=>mapTask(row as TaskRow)),taskDedupeKey)},[] as Task[]),['public-tasks'],{...taskCacheOptions,tags:[DAILY_TASKS_CACHE_TAG]});
...
// 加热 tasks: the newest HEAT_TAB_LIMIT, whose deadline (6 hours by default) is the card's countdown.
const HEAT_TAB_LIMIT=10;
const getCachedHeatTasks=unstable_cache(withFallback(async()=>{const db=createPublicClient();const now=new Date().toISOString();const rows=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('status','published').eq('show_in_heat',true).or(`deadline.is.null,deadline.gt.${now}`).order('created_at',{ascending:false}).limit(HEAT_TAB_LIMIT*2));return dedupeBy((rows??[]).map(row=>mapTask(row as TaskRow)),taskDedupeKey).slice(0,HEAT_TAB_LIMIT)},[] as Task[]),['public-heat-tasks'],taskCacheOptions);
const getCachedTask=unstable_cache(withFallback(async(id:string)=>{const db=createPublicClient();const row=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('id',id).eq('status','published').maybeSingle());return row?mapTask(row as TaskRow):undefined},undefined as Task|undefined),['public-task'],{...taskCacheOptions,tags:[TASK_DETAIL_CACHE_TAG]});
...
// NEW marks everything published on the most recent Beijing date that has any media.
const beijingDate=(iso:string)=>new Date(new Date(iso).getTime()+8*3600e3).toISOString().slice(0,10);
function markNewestDay(items:MediaItem[]){const newest=items.reduce((max,item)=>{const day=beijingDate(item.publishedAt);return day>max?day:max},'');return items.map(item=>({...item,isNew:beijingDate(item.publishedAt)===newest}))}
const getCachedMedia=unstable_cache(withFallback(async()=>{const db=createPublicClient();const cutoff=new Date(Date.now()-MEDIA_EXPIRY_MS).toISOString();const rows=await run(db.from('media_items').select('*').eq('is_enabled',true).gte('published_at',cutoff).order('created_at',{ascending:false}));return markNewestDay(dedupeBy(((rows??[]) as MediaRow[]).map(r=>({id:r.id,title:r.title,category:r.category,publishedAt:r.published_at,coverUrl:r.cover_url??'',url:r.external_url,isNew:false,enabled:r.is_enabled})),mediaDedupeKey))},[] as MediaItem[]),['public-media'],publicCacheOptions);
...
export const repository:ActionHubRepository={
 ...
 getHeatTasks:getCachedHeatTasks,

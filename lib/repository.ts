...
const getCachedTasks=unstable_cache(async()=>{const db=createPublicClient();const rows=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('status','published').eq('show_in_daily',true).order('is_pinned',{ascending:false}).order('daily_sort_position',{ascending:true,nullsFirst:false}).order('deadline',{ascending:true,nullsFirst:false}).order('urgency_score',{ascending:false}).order('required_score',{ascending:false}).order('sort_order'));return (rows??[]).map(row=>mapTask(row as TaskRow))},['public-tasks'],taskCacheOptions);
const getCachedUrgentTasks=unstable_cache(async()=>{const db=createPublicClient();const now=new Date().toISOString();const rows=await run(db.from('tasks').select('*,task_steps(step_number,content)').eq('status','published').eq('show_in_urgent',true).or(`deadline.is.null,deadline.gt.${now}`).order('is_pinned',{ascending:false}).order('urgent_sort_position',{ascending:true,nullsFirst:false}).order('created_at',{ascending:false}));return (rows??[]).map(row=>mapTask(row as TaskRow))},['public-urgent-tasks'],taskCacheOptions);

...
type TaskRow={id:string;title:string;description:string|null;recommended_copy:string|null;category:string;platform:string|null;external_url:string;quick_instruction:string;urgency_score:number;required_score:number;estimated_minutes:number;deadline:string|null;created_at:string;is_pinned:boolean;show_in_urgent:boolean;show_in_daily:boolean;daily_group:DailyGroup;is_multi_account:boolean;completion_mode:'one_time'|'daily';sort_order:number;urgent_sort_position:number|null;task_steps?:{step_number:number;content:string}[]};
const mapTask=(row:TaskRow):Task=>({id:row.id,title:row.title,description:row.description??'',recommendedCopy:row.recommended_copy??'',category:row.category as Category,platform:row.platform??'',url:row.external_url,quick:row.quick_instruction,urgency:row.urgency_score,required:row.required_score,minutes:row.estimated_minutes,deadline:row.deadline,createdAt:row.created_at,pinned:row.is_pinned,urgent:row.show_in_urgent,daily:row.show_in_daily,dailyGroup:row.daily_group,multi:row.is_multi_account,urgentSortPosition:row.urgent_sort_position??null,steps:[...(row.task_steps??[])].sort((a,b)=>a.step_number-b.step_number).map(x=>x.content)});

...
export async function getUserDashboard(next='/me'){
  ...
  if(!timezone)return {user,profile,history,accounts,leaderboard,stats:{today:null,week:null,streak:null,total:(rows??[]).filter(row=>row.counts_toward_stats).length}};
  const valid=(rows??[]).filter(row=>row.counts_toward_stats);let today:string;let keys:string[];
  try{today=localDateKey(new Date(),timezone);keys=valid.map(row=>localDateKey(row.completed_at,timezone))}catch{return {user,profile,history,accounts,leaderboard,stats:{today:null,week:null,streak:null,total:valid.length}}}
  ...
  return {user,profile,history,accounts,leaderboard,stats:{today:keys.filter(key=>key===today).length,week:keys.filter(key=>key>=monday&&key<=today).length,streak,total:valid.length}};
}

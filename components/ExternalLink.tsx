...
import {externalWebHref,externalWebTarget,isWeiboWboxUrl} from '@/lib/url';
...
export function ExternalLink({href,className,children}:{href:string;className?:string;children:React.ReactNode}){
  ...
  const openWeibo=()=>{clear();setFallback(false);setCopied(false);window.location.href=`sinaweibo://browser?url=${encodeURIComponent(safeHref)}`;timer.current=window.setTimeout(()=>{timer.current=null;if(document.visibilityState==='visible')setFallback(true)},1500)};

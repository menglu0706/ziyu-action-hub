...
import {externalWebHref,externalWebTarget,isWeiboWboxUrl,weiboAppBrowserUrl,weiboSinaUrlWrapper} from '@/lib/url';
...
export function ExternalLink({href,className,children}:{href:string;className?:string;children:React.ReactNode}){
  ...
  const wrapper=safeHref?weiboSinaUrlWrapper(safeHref):null,appUrl=safeHref?weiboAppBrowserUrl(safeHref):null;
  ...
  const openFallback=()=>{if(wrapper)window.location.assign(wrapper)};
  ...
  const target=wbox?'_blank':externalWebTarget(safeHref)??undefined,navigationHref=wrapper??safeHref;
  return <><a className={className} href={navigationHref} target={target} rel="noreferrer" onClick={click}>{children}</a>{fallback&&<div className="external-link-modal-backdrop" role="presentation"><section className="external-link-modal" role="dialog" aria-modal="true" aria-labelledby="wbox-fallback-title"><h2 id="wbox-fallback-title">此任务需要在微博 App 内打开</h2><div><button className="primary" onClick={openFallback}>再次打开微博</button><button className="outline" onClick={copy}>{copied?'已复制':'复制任务链接'}</button><button className="plain" onClick={()=>setFallback(false)}>关闭</button></div></section></div>}</>;
}

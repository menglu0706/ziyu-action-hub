...
export function AvatarUpload({userId,avatarUrl}:{userId:string;avatarUrl:string|null}){
  ...
  const upload=async(file?:File)=>{if(!file||busy)return;if(!acceptedTypes.has(file.type)){setMessage('请选择 JPEG、PNG 或 WebP 图片');return}if(file.size>maxSourceBytes){setMessage('原图不能超过 5 MB');return}const previous=preview,localUrl=URL.createObjectURL(file);setPreview(localUrl);setBusy(true);setMessage('');try{const blob=await prepareAvatar(file),path=`${userId}/avatar.webp`,db=createClient(),{error}=await db.storage.from('avatars').upload(path,blob,{contentType:'image/webp',cacheControl:'31536000',upsert:true});if(error)throw new Error('头像上传失败');const savedUrl=await saveAvatarUrl();setPreview(savedUrl);setMessage('头像已更新')}catch(cause){setPreview(previous);setMessage(cause instanceof Error?cause.message:'头像更新失败')}finally{URL.revokeObjectURL(localUrl);setBusy(false);if(input.current)input.current.value=''}};
  return <section className="flex items-center gap-4 rounded-2xl border border-[#e2edf5] bg-[#f8fcff] p-4"><div className="relative grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-full bg-[#dff1fb]">{preview?preview.startsWith('blob:')?
    // eslint-disable-next-line @next/next/no-img-element
    <img src={preview} alt="当前头像" className="h-full w-full object-cover"/>
    :<Image src={preview} alt="当前头像" fill sizes="80px" className="object-cover"/>:<span className="fish"/>}</div><div className="min-w-0 flex-1"><label className={`inline-flex min-h-9 cursor-pointer items-center justify-center rounded-full border border-[#bfd7e7] bg-white px-4 text-xs font-semibold text-[#416d91] ${busy?'pointer-events-none opacity-60':''}`}>{busy?'正在更新…':'更换头像'}<input ref={input} hidden disabled={busy} type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>upload(event.target.files?.[0])}/></label><p className="mb-0 mt-2 text-[10px] leading-4 text-[#8298ac]">JPEG / PNG / WebP，原图最大 5 MB</p>{message&&<p className={`mb-0 mt-1 text-xs ${message==='头像已更新'?'text-[#438bd1]':'text-red-500'}`}>{message}</p>}</div></section>;
}

...
export async function TaskViewerControls({task,session}:{task:Task;session:TaskViewerSession}){
  ...
  return <div className="mt-4 flex items-center justify-between"><span className="text-xs font-bold text-[#7890a6]">多账号进度</span>{typeof goal==='number'?<ProgressControl taskId={task.id} initial={progress} goal={goal}/>:<Link href="/me/account" className="text-xs text-[#438bd1]">请先设置账号数量</Link>}</div>;
}

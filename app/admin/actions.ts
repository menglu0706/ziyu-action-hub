...
export async function setTaskStatus(id:string,status:'published'|'offline'):Promise<ActionResult>{return toResult(async()=>{const {db}=await context();
// Putting an expired 加热 task back online gives it a fresh 6 hours; otherwise the expiry job would take it straight down again.
let deadline:{deadline:string}|null=null;if(status==='published'){const {data:task}=await db.from('tasks').select('show_in_heat,deadline').eq('id',id).maybeSingle();if(task?.show_in_heat&&(!task.deadline||new Date(task.deadline).getTime()<=Date.now()))deadline={deadline:new Date(Date.now()+HEAT_TTL_MS).toISOString()}}
const {error}=await db.from('tasks').update({status,publish_at:null,...deadline}).eq('id',id);if(error)throw new Error('任务状态更新失败');revalidatePath('/admin/tasks');revalidatePath('/urgent');revalidatePath('/heat');revalidatePath('/daily');revalidatePath(`/tasks/${id}`);revalidateTag(TASK_DETAIL_CACHE_TAG)})}
export async function setWatcherEnabled(enabled:boolean):Promise<ActionResult>{return toResult(async()=>{const {db}=await context();const {error}=await db.from('weibo_watcher_settings').update({enabled,updated_at:new Date().toISOString()}).eq('id',true);if(error)throw new Error('微博监控开关更新失败');revalidatePath('/admin')})}
export async function unpinTask(id:string):Promise<ActionResult>{return toResult(async()=>{const {db}=await context();const {error}=await db.from('tasks').update({is_pinned:false,pin_starts_at:null,pin_ends_at:null}).eq('id',id);if(error)throw new Error('取消置顶失败');revalidatePath('/admin/tasks');revalidatePath('/urgent');revalidatePath('/heat');revalidateTag(DAILY_TASKS_CACHE_TAG)})}
...
export async function deleteTasks(ids:string[]):Promise<ActionResult>{return toResult(async()=>{const unique=[...new Set(ids.filter(id=>typeof id==='string'&&id))];if(!unique.length)throw new Error('请先选择任务');if(unique.length>200)throw new Error('一次最多删除 200 个任务');const {db}=await context();const {error}=await db.from('tasks').delete().in('id',unique);if(error)throw new Error('批量删除失败');revalidatePath('/admin/tasks');revalidatePath('/urgent');revalidatePath('/heat');revalidatePath('/daily');for(const id of unique)revalidatePath(`/tasks/${id}`);revalidateTag(TASK_DETAIL_CACHE_TAG);revalidateTag(DAILY_TASKS_CACHE_TAG)})}
